...
namespace CADability.GeoObject
{
    ...
    public class BSpline : IGeoObjectImpl, IColorDef, ILineWidth, ILinePattern, ISerializable, ICurve, IExplicitPCurve3D, IExportStep
    {
        ...
        /// <para>
        /// The parameter of the resulting BSpline is the parameter of the provided function, i.e. it runs from
        /// <paramref name="minPar"/> to <paramref name="maxPar"/>. <paramref name="precision"/> bounds the geometric
        /// distance between the curve and the spline, the distance between the points with the same parameter may be
        /// larger where it is mostly tangential.
        /// </para>
        /// </summary>
        ...
        {
            if (curve == null || !(maxPar > minPar)) return null; // an empty parameter range, the loops below would not terminate
            ...
            Dictionary<double, GeoPoint> samples = new Dictionary<double, GeoPoint>();
            ...
            Nurbs<GeoPoint, GeoPointPole> nurbs = null;
            ...
            while (true)
            {
                GeoPoint[] points = new GeoPoint[nodes.Count];
                ...
                try
                {
                    nurbs = new Nurbs<GeoPoint, GeoPointPole>(points, parameters, 3);
                }
