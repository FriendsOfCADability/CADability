    ...
    internal class TetraederHull
    {
        ...
        public bool HitTest(Projection.PickArea area, bool onlyInside)
        {
            if (onlyInside)
            {
                for (int i = 0; i < tetraederBase.Length; ++i)
                {
                    if (!BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * tetraederBase[i])) return false;
                }
                for (int i = 0; i < tetraederBase.Length - 1; ++i)
                {
                    if (!BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * tetraederVertex[2 * i]) ||
                        !BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * tetraederVertex[2 * i + 1]))
            ...
            else
            {
                for (int i = 0; i < tetraederBase.Length; ++i)
                {
                    if (BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * tetraederBase[i])) return true;
                }
                for (int i = 0; i < tetraederBase.Length - 1; ++i)
                {
                    if (IsLine(i))
                    {
                        ...
                        if (BoundingBox.UnitBoundingBox.Interferes(ref p1, ref p2)) return true;
                    }
                    else if (BoundingBox.UnitBoundingBox.Interferes(area.ToUnitBox * tetraederBase[i], area.ToUnitBox * tetraederBase[i + 1], area.ToUnitBox * tetraederVertex[2 * i], area.ToUnitBox * tetraederVertex[2 * i + 1]))
        ...
        {   // Beschreibung eines Tetraeders mit zugehörigem Kurvenabschnitt
            ...
            {   // überschneiden sich zwei Tetraeder
                ...
                else
                {
                    ...
                    //dc.Add(BoundingBox.UnitBoundingBox.AsBox);
#endif
                    if (!BoundingBox.UnitBoundingBox.Interferes(p1, p2, p3, p4)) return false;
                    // jetzt noch schwieriger Fall: kein Punkt innerhalb, aber auch nicht außerhalb des cubes
                    // eine der 6 Kanten muss schneiden. Wenn man mit dem UnitBoundingBox trimmt, dann muss ein Punkt
                    ...
                    if (BoundingBox.UnitBoundingBox.ClipLine(ref start, ref end))
                    {
...
#endif
            if (BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * pm)) return true; // zwischenpunkt drin, also berührt
            if (BoundingBox.UnitBoundingBox.Interferes(area.ToUnitBox * p1, area.ToUnitBox * pm, area.ToUnitBox * tv1, area.ToUnitBox * tv2))
            {
                // 1. Hälfte nicht eindeutig, muss noch weiter untersucht werden
                if (IsPartInside(area, p1, pm, par1, parm)) return true;
            }
            if (BoundingBox.UnitBoundingBox.Interferes(area.ToUnitBox * pm, area.ToUnitBox * p2, area.ToUnitBox * tv3, area.ToUnitBox * tv4))
            {
        ...
        private bool IsPartOutside(Projection.PickArea area, GeoPoint p1, GeoPoint p2, double par1, double par2)
        {
            ...
            double parm;
            SplitTetraeder(p1, p2, par1, par2, out pm, out parm, out tv1, out tv2, out tv3, out tv4);
            if (!BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * pm)) return true; // zwischenpunkt draußen, also außerhalb
            if (!BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * tv1) ||
                !BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * tv2))
            {
                // 1. Hälfte nicht eindeutig, muss noch weiter untersucht werden
                if (IsPartOutside(area, p1, pm, par1, parm)) return true;
            }
            if (!BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * tv3) ||
                !BoundingBox.UnitBoundingBox.Contains(area.ToUnitBox * tv4))
            {
                // 2. Hälfte nicht eindeutig, muss noch weiter untersucht werden
